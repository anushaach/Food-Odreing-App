...
import my.shopfood.CustomerFoodPanel_BottomNavigation;
import my.shopfood.DeliveryFoodPanel_BottomNavigation;
import my.shopfood.MainActivity;
import my.shopfood.R;
import my.shopfood.chefFoodPanel.Chef_Prepared_Order_View;
import my.shopfood.customerFoodPanel.PayableOrders;
...
public class ShowNotification {
    public static void ShowNotif(Context context, String title, String message, String page) {
        ...
        if (page.trim().equalsIgnoreCase("RejectOrder")) {
            acIntent = new Intent(context, Chef_Prepared_Order_View.class).putExtra("PAGE", "RejectOrderpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.VIBRATE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions((Activity) context, new String[]{Manifest.permission.VIBRATE}, 0);
            return;
        }
